...
public enum PickleJarComponentProvider implements IBlockComponentProvider {
    ...
    private PickleJarComponentProvider() {
    }

    @Override
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig pluginConfig) {
        if (accessor.getBlockState().getValue(PickleJarBlock.FERMENTING)) {
            if (accessor.getBlockEntity() instanceof PickleJarBlockEntity be) {
                ...
                MutableComponent timeText = Component.literal(StringUtil.formatTickDuration(remainingTicks));
